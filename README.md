...
Las capas se organizan como círculos concéntricos, donde las capas internas no saben nada de las externas:

```
        ┌──────────────────────────────────┐
        │         Infraestructura          │  ← Express, DB, APIs externas
        │   ┌──────────────────────────┐   │
...
│   ├── domain/
│   │   └── entities/
│   │       └── holaDominio.js       ← Entidad principal del negocio
│   ├── application/
│   │   └── use-cases/
│   │       └── holaApp.js           ← Lógica que conecta dominio con interfaces
│   └── interfaces/
│       └── controllers/
│           └── controladorSaludo.js ← Punto de entrada, muestra el resultado
├── index.js                         ← Archivo principal, inicia la ejecución
├── package.json
└── README.md
```
...
En este caso, ahí está el archivo `holaDominio.js`, que define la clase `Saludo` con el nombre y el mensaje que se va a mostrar.
Esta capa es la más interna de la arquitectura, por lo mismo no depende de las otras carpetas.

- **application/**
La carpeta application contiene la lógica que conecta el dominio con el resto del programa.
En este proyecto, el archivo `holaApp.js` contiene la clase `ObtenerSaludo`, que toma el mensaje definido en el dominio y lo prepara para que pueda ser usado por la interfaz.

- **interfaces/**
La carpeta interfaces representa la parte que se comunica con el exterior.
En este caso, `controladorSaludo.js` recibe el resultado desde la capa de aplicación y lo muestra por consola.
El controlador está separado en su propia subcarpeta `controllers/` para mayor organización.

- **index.js**
El archivo `index.js` es el archivo principal del proyecto.
Desde aquí parte la ejecución del programa y se llama al controlador para que se muestre el mensaje final.
...
index.js
   ↓
interfaces/controllers/controladorSaludo.js
   ↓
application/use-cases/holaApp.js
   ↓
domain/entities/holaDominio.js
   ↓
...
La regla más importante de Clean Architecture es que las dependencias solo apuntan hacia adentro:

| Capa | Puede importar de... |
|---|---|
| `domain` |  Nadie — es el núcleo independiente |
| `application` |  `domain` |
| `interfaces` |  `application` |
| `infraestructura` |  `domain`, `application` |
...
Para ejecutar el proyecto, primero se deben instalar las dependencias con:
...
Al ejecutar el programa, se mostrará el mensaje:
...
Además, esta estructura permite que si más adelante se quiere cambiar la forma en que se muestra el mensaje, agregar una base de datos o incluir nuevas funciones, no sea necesario modificar todo el proyecto, solo se trabajaría sobre la capa correspondiente.
